...
pub fn show_inventory(gs : &mut State, ctx : &mut Rltk, msg: String) -> (ItemMenuResult, Option<Entity>) {
    ...
    ctx.print_color(18, y-2, RGB::named(rltk::YELLOW), RGB::named(rltk::BLACK), msg);
...
pub fn ranged_targeting(gs: &mut State, ctx: &mut Rltk, range: i32) -> (ItemMenuResult, Option<Point>){
    let player_entity = gs.ecs.fetch::<Entity>();
